...
def movimentar_dinheiro(historico: Historico):
    with Session(engine) as session:
        ...
        session.commit()
        print(f"Movimentação realizada com sucesso: {historico.valor} {historico.tipo} na conta {historico.conta_id}.")
        return historico
...
def total_contas():
    ...
    for conta in contas:
        total += conta.valor
        print(f"Conta {conta.id}: {conta.banco} - Saldo: {conta.valor}")
    return float(total)
...
def criar_grafico_por_conta():
    with Session(engine) as session:
        ...
        plt.title('Valor por Banco (Contas Ativas)')

        # Salvar o gráfico como uma imagem
        plt.savefig('grafico_contas.png')
        print("Gráfico salvo como 'grafico_contas.png'")
...
# conta= Conta(valor=10,banco="INTER")
# criar_conta(conta)
...
# transferir_saldo(1,2,valor=20)

# transferir_saldo(1, 2, 5)  # Transfere 5 da conta 1 para a conta 2
